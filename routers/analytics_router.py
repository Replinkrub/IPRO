...
from services.models import CustomerAnalytics, KPIResponse
...
def _compute_kpis_from_payload(payload: dict) -> dict:
    ...
    # Recency
    rec_col = next((c for c in ["recency", "R_recency_days", "R_days"] if c in cm.columns), None)
    avgR = int(round(_safe_mean(cm[rec_col]))) if rec_col else 0

    # Frequency
    freq_col = next((c for c in ["order_count", "orders", "F_count", "freq", "frequency"] if c in cm.columns), None)
    avgF = round(_safe_mean(cm[freq_col]), 1) if freq_col else 0.0

    # Avg Ticket: tenta coluna direta; senão, monetary / orders
    tick_col = next((c for c in ["avg_ticket", "ticket_medio"] if c in cm.columns), None)
    if tick_col:
        avgM = float(round(_safe_mean(cm[tick_col]), 2))
    else:
        mon_col = next((c for c in ["monetary", "total_value"] if c in cm.columns), None)
...
async def analyze_customers(dataset_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        avg_recency = int(sum(ca.recency for ca in customer_analytics) / total_customers) if total_customers > 0 else 0
        avg_frequency = float(sum(ca.frequency for ca in customer_analytics) / total_customers) if total_customers > 0 else 0.0
        avg_value = float(sum(ca.monetary for ca in customer_analytics) / total_customers) if total_customers > 0 else 0.0
        
        res = {
            "customer_analytics": [ca.dict() for ca in customer_analytics]
        }
        res["kpis"] = _compute_kpis_from_payload(res)
        return res
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro na análise de clientes: {str(e)}")
...
async def get_kpis(dataset_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro no cálculo de KPIs: {str(e)}")
...
async def get_top_products(dataset_id: str, by: str = "receita", db=Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro no ranking de produtos: {str(e)}")
    ...
    try:
        ...
        for segment, data in segments.items():
            result[segment] = {
                ...
                "avg_ticket": data["revenue"] / data["orders"] if data["orders"] > 0 else 0
            }
...
async def analyze_products(dataset_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        calculator = MetricsCalculator()
        product_analytics = calculator.calculate_product_analytics(transactions, dataset_id)
        ...
        raise HTTPException(status_code=500, detail=f"Erro na análise de produtos: {str(e)}")
...
def _load_transactions_df(dataset_id: str) -> pd.DataFrame:
    ...
    try:
        ...
        transactions = list(db.transactions.find({"dataset_id": dataset_id}))
        if not transactions:
            return pd.DataFrame(columns=["data_emissao","pedido","cliente","criador","preco_liquido","quantidade","subtotal","produto"])
...
def get_dataset_summary(datasetId: str):
    ...
    if df.empty:
        return JSONResponse({
            "n_clientes": 0, "n_cnpjs": 0, "ufs": [], "cidades_top": [],
    ...
    if "categoria" in df.columns:
        categorias_top = (
            df["categoria"].dropna().astype(str)
              .value_counts().head(10).index.tolist()
        )
...
def _compute_rico_alerts(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for (cli, prod), g in df.groupby(["cliente","produto"], dropna=True):
        ...
        deltas = pd.Series(dts[1:] - dts[:-1]).dt.days if len(dts) > 1 else pd.Series([np.nan])
        ...
        status = "OK"
        if today >= ultimo + pd.Timedelta(days=int(0.75*(giro_mediano + DEFAULT_DELAY_LOGISTICO))) and today < limite:
            status = "ALERTA"
        if today > limite:
            status = "CRITICO"
    ...
    for cli, g in df.groupby("cliente"):
        g = g.sort_values("data_emissao")
        c_atual = g[g["data_emissao"] >= (today - pd.Timedelta(days=win))]["subtotal"].sum()
        c_ant = g[(g["data_emissao"] < (today - pd.Timedelta(days=win))) & (g["data_emissao"] >= (today - pd.Timedelta(days=2*win)))]["subtotal"].sum()
    ...
    if not once.empty:
        last_buy = (df
                    ...
                    .drop_duplicates(subset=["cliente","produto"], keep="last")
