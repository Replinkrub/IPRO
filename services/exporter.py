...
class ExcelExporter:
    ...
    def export_to_excel(self, file_path: str, transactions: List[Dict], 
                       customer_analytics: List[Dict], product_analytics: List[Dict], 
                       alerts: List[Dict], dataset_id: str):
        ...
        try:
            ...
            # Aba 1: Painel (KPIs)
            self._create_painel_sheet(workbook, formats, transactions, customer_analytics)
            
            # Aba 2: Radar Cliente
            self._create_radar_cliente_sheet(workbook, formats, customer_analytics)
            
            # Aba 3: Radar Produto
            self._create_radar_produto_sheet(workbook, formats, product_analytics, transactions)
    ...
    def _create_radar_cliente_sheet(self, workbook, formats, customer_analytics):
        ...
        # Cabeçalhos
        headers = ['Cliente', 'Recência (dias)', 'Frequência', 'Valor Monetário', 
                  'Ticket Médio', 'Giro Médio', 'Tier']
    ...
    def _create_radar_produto_sheet(self, workbook, formats, product_analytics, transactions):
        ...
        # Cabeçalhos
        headers = ['SKU', 'Produto', 'Receita Total', 'Quantidade Total', 
                  'Pedidos', 'Clientes', 'Penetração %']
        ...
        for row, (_, product) in enumerate(product_stats.iterrows(), 1):
            penetration = (product['client'] / total_customers * 100) if total_customers > 0 else 0
    ...
    def _create_alvos_priorizados_sheet(self, workbook, formats, alerts):
        ...
        # Cabeçalhos
        headers = ['Cliente', 'SKU', 'Tipo', 'Diagnóstico', 'Ação Recomendada', 
                  'Confiabilidade', 'Prazo']
        ...
        for row, alert in enumerate(alerts, 1):
            ...
            worksheet.write(row, 6, alert.get('suggested_deadline', ''), formats['text'])
    ...
    def _create_base_completa_sheet(self, workbook, formats, transactions):
        ...
        # Cabeçalhos
        headers = ['Data Emissão', 'Pedido', 'Cliente', 'Vendedor', 'SKU', 'Produto', 
                  'Preço Unitário', 'Quantidade', 'Subtotal', 'Categoria', 'Segmento', 
                  'Cidade', 'UF']
