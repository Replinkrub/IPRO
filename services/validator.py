...
class DataValidator:
    ...
    def _validate_transaction(
        self, transaction: Dict[str, Any], index: int
    ) -> Dict[str, Any]:
        ...
        if abs(validated["subtotal"] - expected_subtotal) > 0.01:
            logger.warning(
                f"Subtotal inconsistente na linha {index}: "
    ...
    def validate_customers(
        ...
        if self.errors:
            logger.warning(
                f"Encontrados {len(self.errors)} erros de validação de clientes"
            )
    ...
    def _validate_customer(
        self, customer: Dict[str, Any], index: int
    ) -> Dict[str, Any]:
