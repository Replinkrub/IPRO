...
class ProReportBuilder:
    ...
    def build(
        ...
        with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
            ...
            fmt_header = workbook.add_format({'bold': True, 'bg_color': '#1F4E78', 'font_color': 'white'})
            fmt_text = workbook.add_format({'text_wrap': True})

            self._write_base_completa(base_df, writer, fmt_date, fmt_currency, fmt_header)
            self._write_potencial_cliente(clientes_df, writer, fmt_header, fmt_currency)
            self._write_potencial_produto_cliente(base_df, writer, fmt_header, fmt_currency)
            self._write_insights(alertas_df, writer, fmt_header, fmt_text)
            self._write_alertas(alertas_df, writer, fmt_header, fmt_text)

    def _write_base_completa(self, df: pd.DataFrame, writer, fmt_date, fmt_currency, fmt_header):
        ...
        for idx, col in enumerate(df.columns):
            ...
            width = max(12, int(df[col].astype(str).str.len().mean() * 1.3)) if not df.empty else 15
            sheet.set_column(idx, idx, width, fmt_date if 'date' in col or 'data' in col else None)
        if 'subtotal' in df.columns:
            col_idx = df.columns.get_loc('subtotal')
            sheet.set_column(col_idx, col_idx, 18, fmt_currency)

    def _write_potencial_cliente(self, df: pd.DataFrame, writer, fmt_header, fmt_currency):
        sheet_name = 'Potencial do Cliente'
        if df.empty:
            df = pd.DataFrame(columns=['client', 'rfm_score', 'segment', 'avg_ticket', 'gm_cliente', 'tier'])
        df_to_write = df.copy()
        if 'last_order' in df_to_write.columns:
            df_to_write['last_order'] = pd.to_datetime(df_to_write['last_order'], errors='coerce')
        df_to_write.sort_values(by='rfm_score', ascending=False, inplace=True, na_position='last')
        df_to_write.to_excel(writer, sheet_name=sheet_name, index=False)
        sheet = writer.sheets[sheet_name]
        sheet.freeze_panes(1, 0)
        for idx, col in enumerate(df_to_write.columns):
            sheet.write(0, idx, col, fmt_header)
            width = max(12, int(df_to_write[col].astype(str).str.len().mean() * 1.2)) if not df_to_write.empty else 15
    ...
    def _write_potencial_produto_cliente(self, df: pd.DataFrame, writer, fmt_header, fmt_currency):
        ...
        for idx, col in enumerate(pivot.columns):
            sheet.write(0, idx, col, fmt_header)
            width = max(12, int(pivot[col].astype(str).str.len().mean() * 1.2)) if not pivot.empty else 15
    ...
    def _write_alertas(self, df: pd.DataFrame, writer, fmt_header, fmt_text):
        ...
        if df.empty:
            alerts = pd.DataFrame(columns=['client', 'sku', 'type', 'insight', 'action', 'reliability', 'suggested_deadline'])
        else:
            alerts = df.copy()
        ...
        for idx, col in enumerate(alerts.columns):
            sheet.write(0, idx, col, fmt_header)
            width = max(12, int(alerts[col].astype(str).str.len().mean() * 1.1)) if not alerts.empty else 18
            sheet.set_column(idx, idx, width, fmt_text)
