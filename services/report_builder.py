...
def build_report_dataframes(
    ...
    if not transactions:
        raise ValueError(
            "Nenhuma transação normalizada disponível para gerar relatórios"
        )
    ...
    clientes_df = (
        pd.DataFrame([c.model_dump() for c in customer_analytics])
    ...
    mix_df = (
        pd.DataFrame([p.model_dump() for p in product_analytics])
    ...
    if not clientes_df.empty:
        ...
        relacional_df["last_order"] = pd.to_datetime(
            relacional_df["last_order"], errors="coerce"
        )
        relacional_df["janela_prevista_dias"] = (
            relacional_df["gm_cliente"].fillna(0) + calc.delay_logistico
        )
    ...
    behavior_rows = [
        {
            ...
            "valor": general_kpis.get("total_customers", 0),
        },
        ...
        {"indicador": "Total de pedidos", "valor": general_kpis.get("total_orders", 0)},
        {
            ...
            "valor": round(general_kpis.get("avg_ticket", 0.0), 2),
        },
        {
            "indicador": "Ruptura projetada média (dias)",
            "valor": round(general_kpis.get("ruptura_projetada_media", 0.0), 2),
        },
    ]
...
def write_report_excel(
    dataframes: Dict[str, pd.DataFrame], engine: str = "xlsxwriter"
) -> str:
